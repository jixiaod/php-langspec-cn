...
最后两个变量中的*简单变量*或*表达式*必须指明一个标量值或可转换为字符串的对象。
...
*简单变量*表达式指明具有由*变量名*或*简单变量*或*表达式*的结果的字符串表示形式确定的名称的变量，
这取决于哪种情况适用。 在后两种情况下，[变量名](09-lexical-structure.md#names)可以包含在词法变量名中不允许的字符。

*简单变量*在不同上下文和不同类型变量中的行为在[变量](07-变量.md)部分中指明。
